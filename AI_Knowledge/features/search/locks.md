...
LOCKED | Sorting and filtering are server capabilities — never re-sort or re-filter a page of results client-side | Faking a capability over 20 loaded rows lies to the user. Missing param → raise it (P9.4). **REAFFIRMED 2026-08-02 by owner decision** (PRODUCT_VISION §4 append), against backend prose that described the opposite: filtering and sorting run on the SERVER across the whole catalogue, always. A missing server param now means the control WAITS — a client-side refinement layer is not an available fallback | search api.ts, filter/sort controls
LOCKED | Results load by infinite scroll, and changing any filter or sort RESETS the list | 2026-08-02, owner. There is no customer-visible pagination on any viewport. A refinement is a new question, not a narrowing of the current answer — so the accumulated pages are discarded and the server is re-queried from page 0, never spliced. The `store.ts` this requires holds honest server pages and must never become a place to filter them | search store.ts, CatalogPage, filter/sort controls
