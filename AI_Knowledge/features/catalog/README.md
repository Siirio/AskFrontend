...
- **"Proceed to Purchase" resolved 2026-08-02 (G3, PRODUCT_VISION UF 2.1 append):** seller-supplied public **deeplinks** → a chooser **modal** when there is more than one → the in-app chat with an **editable, never-auto-sent draft** when there are none. The two buttons stay distinct: chat asks a question, this one states intent to buy. Verification links (`kaspiUrl`/`ozonUrl`/`wildberriesUrl`, which live on `BusinessVerification`) are never reused as customer deeplinks. **Backend-blocked** — `deepLink` is a single `String` on `Item`, does not exist on `Service`, and is absent from the search projection; the button is omitted until it arrives.
