...
- **P1.1a** *(added 2026-08-01)* **SCOPE — the cap is about COMPONENT files, and only those.** A slice's `api.ts`, `model.ts` and `hooks.ts` are modules whose length tracks the size of the DOMAIN's surface, not a screen's complexity: `model.ts` holds the DTO types and mappers that P4.1 says belong exactly there, and `hooks.ts` holds one entry per flow. They are governed by **P1.3** (one responsibility per hook) and **P1.4** (do not over-split), never by a line count. Splitting them would also change the slice anatomy, which architecture §3 fixes on purpose. **Why this is written down:** the cap was applied to `hooks.ts`/`model.ts` twice — AUDIT_1 B8, then the 2026-08-01 audit — and actioned neither time, because each reading was arguable and the fix was worse than the finding. A rule that keeps producing findings nobody acts on is worse than no rule: it trains readers to skip the list. If one of these files becomes hard to navigate, the answer is ordering and section comments inside it, or a hook that is genuinely two responsibilities (P1.3) — not a folder. **Justification:** owner decision 2026-08-01, taken deliberately before slices #3–#6 were written, so the next four slices inherit a settled shape rather than a recurring argument.
