...
| 3 | `catalog` | **Customer half SHIPPED 2026-08-06.** Product Card — modal over the catalog, rendered from the search card payload (zero new requests). **`/app/product/:id` is permanently modal-only (owner, 2026-08-06, D33) — no page will be built.** The up-to-3 image gallery and "Proceed to Purchase" (gate G3, closed 2026-08-04) both ship; the chat button and its zero-destination fallback are deferred to slice #4 (`catalog/locks.md`). **The seller half (Products tab, import) is NOT built** — that is roadmap item #7 | UF 2.1 · 3–4 |
...
| ~~G1 (superseded)~~ | **ARCHIVED 2026-08-04.** The 2026-07-28 narrowing and its "still missing" list described a wire that no longer exists — every item on it shipped. Full history in `Changelog.md`; the live status is the **G1 — CLOSED** row above. Kept as one line rather than deleted, because a reader who remembers the parked controls needs to find out they were delivered, not find silence |  |  |
...
| ~~G3 (superseded)~~ | **ARCHIVED 2026-08-04.** Two rows lived here: the 2026-07-28 re-opening (its `contact`-module candidate was dead) and the 2026-08-02 status update (product agreed, backend not built). Both are now history — `c56f75c` shipped `PurchaseDestination` on Item, Service AND the search projection. Live status is the **G3 — CLOSED** row above |  |  |
...
| Need | For | Status |
|---|---|---|
| ~~Confirm that `search/README.md`'s "match reasons … are not displayed" is a note about the BACKEND's own UI, not a product change~~ | ~~The result card's "why this matched" list~~ | **RESOLVED 2026-08-06 (owner) — REVERSES the 2026-08-02 ruling on this same row.** `b02105a` (2026-08-02) rewrote the backend's § *Result presentation* to say match reasons *"remain response metadata but are not displayed"*. The first ruling explicitly overrode that and kept them rendered as the intent layer's core affordance. **That override is now itself reversed**: match reasons are NOT rendered anywhere in the product. `search/model.ts`'s `matchReasons` field stays modelled (still on the wire, backend remains the data authority) but nothing renders it; `ResultCard.tsx`'s rendering block was deleted the same day. The client now agrees with the backend's own README instead of overriding it. Kept per this file's own rule — a resolved row stays, it does not vanish. **The rest of that section — hover-preview panel, mobile detail modal — remains not adopted**; only the primary card image was, via the 2026-08-04 vision append |
...
| ~~A public item/service read — `GET /api/v1/items/{id}` (or equivalent)~~ | ~~Slice #3's `/app/product/:id` deep link~~ | **WITHDRAWN 2026-08-06 (D33).** The need this row raised no longer exists: the Product Card is modal-only, permanently — there will never be a full-page presentation to serve, so no public item read is being asked for. Kept per this file's own rule (a closed/withdrawn row stays, it does not vanish). There is still NO public item endpoint on `dev` as of this writing (the security allowlist permits only `/search`, `/cities`, `/categories`, `/businesses/*/business-profile`, `/businesses/*/drops`, `/business-media/files/*`), which is one of the two reasons D33 gave for making the reversal — that fact just no longer blocks anything |
