...
**§8 note — 2026-08-01 (the check's own blind spot, closed same day):** review of `check-tokens.mjs` found it shipping the defect it exists to prevent. Its token regex demanded exactly three numbers, so `oklch(0 0 0 / 0.45)` never parsed — silently excluding the three shadow colours and `--overlay` from a drift check whose own output says "token by token" (21 of 24 tokens covered per block). Demonstrated rather than argued: drifting a shadow alpha in the no-JS block printed **PROVEN … both dark blocks in step**, a false green in precisely the blind spot the script was written to cover. Alpha is now parsed (defaulting to 1) and compared alongside L/C/H, the failure message prints alpha whenever either side carries it — a pure-alpha drift would otherwise render as two identical-looking values — and `rgbOf` now REFUSES a translucent token as one side of a documented pair rather than measuring it as if opaque, since compositing is not modelled and a confident wrong number is worse than a stop. Separately, `PAIRS` keyed on the table's raw label, which coupled semantic identity to column ALIGNMENT: removing one padding space reported the same row as both "unknown to this script" and "absent from the table". Keys are now matched on collapsed whitespace. **The lesson is the reusable part: a check is only as good as its parser, and "it passes" is not evidence — every defect class it claims to catch must be shown failing.** All four now are: a ratio that no longer matches, a miscounted summary, an opaque drift, and an alpha drift.
...
| D32 | 2026-08-04 | **`search/` gains a `store.ts`, and its result list becomes a CLIENT island — both forced by infinite scroll** (PRODUCT_VISION §4, owner 2026-08-02; unblocked when backend `c56f75c` removed `MAX_CANDIDATES` and `page @Max(20)`). Three parts. (1) *The store.* `hooks.ts` opened by stating nothing in search outlives one render because the URL is the source of truth. That is still true of query/mode/sort/filters — every one still travels in the URL and still triggers a fresh SERVER render — but it stopped being true of the RESULTS, because pages 1..n arrive after the render that produced page 0. **This does not weaken the server-capability lock:** the lock forbids REFINING results client-side, and the store only ever concatenates pages the server already answered with the same filters and sort. It never re-sorts, re-filters or merges sections by `kind`, because the backend sections each page independently and merging would reorder cards the server ranked. (2) *The client island.* `ResultSection` and `ResultCard` moved from server to client components, since an appended page must render in the browser and a second client-side copy of the card would be two implementations of one object (P6.1). Page 0 is still fetched server-side by the route file (D7) and arrives inside the island's initial HTML, so first paint and the one-request-if-you-never-scroll behaviour are unchanged; nothing under `/app/*` is crawlable anyway (D23), so no SEO is traded. (3) *The reset is STRUCTURAL.* The vision requires that changing any filter or sort discards the list; the island is mounted under a key derived from the URL params, so a new query gets a new component and a new store rather than a `reset()` that a future control could forget to call. The store therefore has no reset action, and the absence is deliberate | Accepted |
