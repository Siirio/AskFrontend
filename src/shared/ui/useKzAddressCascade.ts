...
/**
 * The cascade STATE behind `AddressSelect` — the KATO registry walk (region →
 * district/settlement, or republican city → city district), its async
 * locality-chunk loading, and the seed-once rehydration from a remembered
 * place. Extracted from the component (2026-08-06, P1.1) so that file stays
 * render-only; this file owns the state, the effects, and the derived value.
 *
 ...
 *
 * The optional `value` seeds the cascade from a REMEMBERED place on mount
 * only — it is read once (a ref guards it) and never re-applied. This is
 * deliberately NOT the usual React "controlled value" contract: passing a
 ...
 * stale copy of what this hook already emitted must never overwrite the
...
type OblastPick =
  | { kind: "district"; item: KatoItem }
  | { kind: "locality"; item: KatoItem };
...
export function useKzAddressCascade(
  value: KzPlace | null,
  ...
  useEffect(() => {
    ...
    // snapshot of what this hook itself emitted earlier. The empty dep
  ...
  const place = useMemo<KzPlace | null>(() => {
    ...
    if (isRepublicanCity(region.id)) {
      ...
      if (chunkState.status === "ready") {
        ...
        // SURFACED next to the field (by the component) rather than swallowed
        // — blocking the seller entirely over a chunk that will not load is
        // the worse of the two failures, but pretending nothing happened is
        // not an option.
        complete = true;
      } else {
