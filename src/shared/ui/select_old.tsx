...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        "focus-ring-field flex w-fit items-center justify-between gap-2 rounded-sm border border-border-strong bg-surface-raised px-3 py-2 text-sm whitespace-nowrap shadow-xs transition-[color,border-color,box-shadow] duration-(--duration-base) ease-out disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive data-placeholder:text-foreground-subtle data-[size=default]:h-9 data-[size=sm]:h-8 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-2 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-foreground-subtle",
        className,
      )}
      {...props}
    >
...
function SelectLabel({
  ...
  return (
    <SelectPrimitive.Label
      ...
      className={cn("px-2 py-1.5 text-xs text-foreground-subtle", className)}
      {...props}
    />
  );
}
